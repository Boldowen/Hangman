    ...
    const words = [
        ...
        { word: "СОРХАГТАН БЭХИ", hint: "Тулуйн хатан, Мөнх, Хубилай, Хүлэгү, Аригбөх нарын эх" },
        { word: "ШИХИХУТАГ", hint: "Чингис хааны өргөмөл дүү, Их Засаг хуулийг бичсэн гэгддэг" },
        { word: "ТООРИЛ ВАН", hint: "Хэрэйдийн хан, Тэмүжиний эцгийн анд, хожмын өрсөлдөгч" },
    ...
    function init() {
        ...
        revealLetterBtn.addEventListener('click', () => {
            if (isGameActive && canRevealLetter && currentWord) {
                revealOneLetter();
            }
        });
    ...
    function resetGame() { // Called before starting a fresh game session or new word
        wrongGuesses = 0;
        guessedLetters = [];
        ...
        if (timerInterval) clearInterval(timerInterval); 
        isGameActive = false; 
         // Score should be reset if it's a completely new game (e.g. after Game Over and Play Again)
        // For this iteration, `score` is reset in `playAgainBtn` and `startGameBtn`'s `startGame` call.
    }
    // Specific function to reset score when a new "overall" game starts
    ...
    function pickRandomWord() {
        ...
        revealLetterBtn.disabled = false;
        revealLetterBtn.textContent = 'Үсэг Илрүүлэх (1)';
    }

    function resetGameForNewWordInternal() { // Renamed to avoid conflict
        wrongGuesses = 0;
        guessedLetters = [];
        hangmanParts.forEach(part => part.style.display = 'none');
        wordStatusEl.textContent = '';
            ...
            if (checkWinCondition()) {
                ...
                setTimeout(() => {
                    if(timeRemaining > 0) { 
                        ...
                        endGame(); // Timer might have run out during the 1.5s delay
                    }
                }, 1500); 
            }
        } else {
    ...
    function revealOneLetter() {
        if (!canRevealLetter || !currentWord) return;

        let unrevealedLetters = [];
        for (let i = 0; i < currentWord.length; i++) {
            if (currentWord[i] !== '_' && !guessedLetters.includes(currentWord[i])) {
                unrevealedLetters.push({letter: currentWord[i], index: i});
            }
        }

        if (unrevealedLetters.length > 0) {
            const letterToReveal = unrevealedLetters[Math.floor(Math.random() * unrevealedLetters.length)].letter;
            
            guessedLetters.push(letterToReveal);
            const keyButton = document.querySelector(`.key[data-letter="${letterToReveal}"]`);
            if (keyButton) {
            ...
            revealLetterBtn.textContent = 'Hint (0)';

            if (checkWinCondition()) {
                score++; 
                scoreEl.textContent = score;
                wordStatusEl.textContent = 'Зөв таалаа (Hint)!';
                 isGameActive = false; 
                setTimeout(() => {
                    if(timeRemaining > 0) {
                        isGameActive = true;
    ...
    // Event listener for startGameBtn (modified for score reset)
    startGameBtn.addEventListener('click', () => {
        if (nicknameInput.value.trim() !== '') {
            nickname = nicknameInput.value.trim();
            setCookie('hangman_nickname', nickname, 30);
